export const MAX_PAGE_LINES = 10000;
export const MAX_PAGE_UTF8_BYTES = 524288; // 512 KB
...
const getUtf8Bytes = (value = '') => utf8Encoder.encode(String(value || '')).length;

export const validatePageContentLimits = ({ note = '', content } = {}) => {
  ...
  if (lineCount > MAX_PAGE_LINES) {
    return {
      ...
      lineCount,
      totalBytes: 0,
      error: `Page content exceeds the ${MAX_PAGE_LINES.toLocaleString()} line limit (${lineCount.toLocaleString()} lines).`,
    };
  }

  const noteBytes = getUtf8Bytes(note);
  const contentBytes = getUtf8Bytes(serializeContent(content));
  ...
  if (totalBytes > MAX_PAGE_UTF8_BYTES) {
    return {
      ...
      lineCount,
      totalBytes,
      error: `Page content exceeds the 512KB limit (${(totalBytes / 1024).toFixed(1)}KB).`,
    };
  }
